...
export function FeedPage() {
  ...
  const load = async () => {
    ...
    try {
      setItems(listify<FeedPost>(await coachApi.feed(gymId)));
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      {!loading && !error && items.length === 0 ? (
        <EmptyState title="پستی نیست" hint="اولین پست را بگذارید" />
      ) : null}
        ...
        {items.map((p) => (
          <div key={p.id} className="card space-y-2">
            <div className="flex items-start justify-between gap-2">
              <div>
                <p className="text-[13px] font-bold text-white">{p.author_name || p.student_name || "مربی"}</p>
                <p className="text-[11px] text-white/35">
                  {p.created_at ? formatFaDate(p.created_at, { short: true }) : ""}
                </p>
              </div>
              <button type="button" className="rounded-lg p-1.5 text-red-300/60" onClick={() => onDelete(p.id)}>
                <Trash2 size={14} />
              </button>
            </div>
            {(p.caption || p.text) ? (
              <p className="text-[13px] leading-relaxed text-white/80">{p.caption || p.text}</p>
            ) : null}
            {p.image ? <img src={p.image} alt="" className="max-h-56 w-full rounded-xl object-cover" /> : null}
            <div className="flex items-center gap-3 pt-0.5 text-[11px] text-white/45">
              <button type="button" className="flex items-center gap-1" onClick={() => onLike(p.id)}>
                <Heart size={13} className={p.liked_by_me || p.is_liked ? "fill-primary text-primary" : ""} />
                {p.likes_count ?? 0}
              </button>

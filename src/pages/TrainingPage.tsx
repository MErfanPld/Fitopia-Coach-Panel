...
export function TrainingPage() {
  ...
  const load = async () => {
    ...
    try {
      const [t, s] = await Promise.all([
        coachApi.training(gymId, { year, month }),
  ...
  const onDelete = async (id: number) => {
    ...
    try { await coachApi.deleteTraining(gymId, id); await load(); }
    catch (err) { alert(err instanceof Error ? err.message : "خطا"); }
  };

  return (
    <PageShell title="برنامه تمرینی" subtitle={`${year}/${month}`}
      ...
      {!loading && !error && items.length === 0 ? <EmptyState title="برنامه‌ای برای این ماه نیست" /> : null}
      <div className="space-y-2">
        {items.map((p) => (
          <div key={p.id} className="card flex items-start gap-3">
            <div className="min-w-0 flex-1">
              <p className="font-bold text-white">{p.title || "برنامه تمرینی"}</p>
              <p className="text-xs text-white/45">{p.student_name || `شاگرد #${p.student}`}</p>

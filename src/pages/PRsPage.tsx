...
export function PRsPage() {
  ...
  const [form, setForm] = useState({ student: "", exercise: "", value: "", unit: "kg", achieved_at: new Date().toISOString().slice(0, 10) });

  const load = async () => {
    if (!gymId) return;
    setLoading(true); setError(null);
    try {
      const [p, s, e] = await Promise.all([coachApi.prs(gymId), coachApi.students(gymId, true), coachApi.exercises(gymId)]);
  ...
  const onDelete = async (id: number) => {
    ...
    try { await coachApi.deletePr(gymId, id); await load(); }
    catch (err) { alert(err instanceof Error ? err.message : "خطا"); }
  };

  return (
    <PageShell title="رکوردهای شخصی" subtitle={`${items.length} رکورد`}
      ...
      <div className="space-y-2">
        {items.map((pr) => (
          <div key={pr.id} className="card flex items-center gap-3">
            <div className="min-w-0 flex-1">
              <p className="font-bold text-white">{pr.exercise_name || `حرکت #${pr.exercise}`}</p>
              <p className="text-xs text-white/45">{pr.student_name || `شاگرد #${pr.student}`} · {pr.achieved_at || pr.date || "—"}</p>
            </div>
            <span className="text-sm font-black text-primary">{pr.value ?? pr.weight} {pr.unit || "kg"}</span>
            <button type="button" className="rounded-lg p-2 text-red-300/70" onClick={() => onDelete(pr.id)}><Trash2 size={16} /></button>
          </div>
        ))}
      </div>
      <Modal open={modal} onClose={() => setModal(false)} title="رکورد جدید">
        <form onSubmit={onCreate} className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div><label className="mb-1 block text-xs text-white/50">مقدار *</label>
              <input className="field" dir="ltr" required value={form.value} onChange={(e) => setForm((f) => ({ ...f, value: e.target.value }))} /></div>
            <div><label className="mb-1 block text-xs text-white/50">واحد</label>
              <select className="field" value={form.unit} onChange={(e) => setForm((f) => ({ ...f, unit: e.target.value }))}>
                <option value="kg">kg</option><option value="reps">reps</option><option value="seconds">ثانیه</option>
              </select></div>
          </div>
          <div><label className="mb-1 block text-xs text-white/50">تاریخ</label>
            <input className="field" type="date" dir="ltr" value={form.achieved_at} onChange={(e) => setForm((f) => ({ ...f, achieved_at: e.target.value }))} /></div>
          <button type="submit" disabled={saving} className="btn btn-primary w-full">{saving ? "…" : "ثبت رکورد"}</button>
        </form>
      </Modal>
    </PageShell>
  );
